...
fn run(config_override: Option<String>) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if input.starts_with('/') {
            match handle_slash(input, &skill_manager, &mut agent) {
                Ok(should_quit) => {
                    if should_quit {
                        break;
                    }
...
fn handle_slash(input: &str, skill_manager: &SkillManager, agent: &mut Agent) -> anyhow::Result<bool> {
